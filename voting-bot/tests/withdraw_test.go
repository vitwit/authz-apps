...
func setup(t *testing.T) (types.Context, lensclient.ChainClient) {
	...
	msg := &banktypes.MsgSend{
		FromAddress: os.Getenv("grantee_addr"),
		ToAddress:   os.Getenv("val_addr"),
		Amount:      sdk.NewCoins(sdk.NewCoin("stake", sdk.NewInt(100))),
	}
	res, err := chainClient.SendMsg(context.Background(), msg, "")
	if err != nil {
		fmt.Println("Err send message, err: ", err)
	}
	fmt.Println("RES send message: ", res)
